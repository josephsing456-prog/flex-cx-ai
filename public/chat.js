// Chat UI Handler for Flex CX
// Manages all chat interactions, file uploads, and message display
...
window.ZexChat = {
  ...
  handleSend: async () => {
    const input = document.getElementById('userInput');
    if (!input || (!input.value.trim() && !window.ZexChat.attachedFile) || window.ZexChat.isLoading) return;
    ...
    try {
      ...
      window.ZexChat.appendBubble(errorMsg, 'ai');
      console.error('[Chat Error]:', err);
    } finally {
      window.ZexChat.isLoading = false;
    }
  },
  ...
  appendBubble: (text, sender, fileData = null, fileName = null) => {
    ...
    if (fileData) {
      ...
      } else if (fileData.startsWith('data:application/pdf') || fileData.startsWith('data:text') || fileData.includes('application/msword') || fileData.includes('application/vnd.openxmlformats')) {
    ...
    if (sender === 'user') {
      ...
      contentHtml = contentHtml.replace(/```(\w*)([\s\S]*?)```/g, (_, lang, code) => {
        return `<pre class="code-block"><div class="code-header"><span>${lang || 'code'}</span><button onclick="window.ZexUtils.copyToClipboard(this.parentElement.nextElementSibling.innerText, this)" class="copy-btn">Copy</button></div><code>${code.trim()}</code></pre>`;
      });
